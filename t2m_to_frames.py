# t2m_to_frames.py
# Reads t2m_2020.nc, extracts 2t, and either:
...
# Color scale (assumes 2t is Kelvin, like ERA5).
MIN_C = -40.0
MAX_C =  40.0
...
def start_ffmpeg_writer(width: int, height: int, fps: int, out_path: Path) -> subprocess.Popen:
    ...
    We send rawvideo frames in RGBA (width*height*4 bytes per frame) to stdin,
    and ffmpeg encodes to H.264 MP4 with yuv420p pixel format.
    """
    ...
    cmd = [
        ...
        "-f", "rawvideo",             # input is raw frames, not an image file
        "-pix_fmt", "rgba",           # the raw frame pixel format we send
        "-s", f"{width}x{height}",    # frame size
        "-r", str(fps),               # input frame rate
        "-i", "pipe:0",               # read frames from stdin
        "-c:v", "libx264",            # encode with H.264
        "-pix_fmt", "yuv420p",        # widely compatible MP4 pixel format
...
def main() -> None:
    ...
    # OUT_DIR derived from VAR_NAME as requested
    out_dir = Path(f"./{VAR_NAME}_images")

    # Open lazily; we'll pull one time slice at a time.
    ...
    # If streaming: determine frame size from the first slice and start ffmpeg once.
    if STREAM_TO_VIDEO:
    ...
    # Main loop: one frame per timestep in chronological order
    for i, t in enumerate(times):
        # Pull one 2D field: (lat, lon)
        ...
        if STREAM_TO_VIDEO:
            ...
            # filename with sequence + timestamp (sequence ensures chronological order)
