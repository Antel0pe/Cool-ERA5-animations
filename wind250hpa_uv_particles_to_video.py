...
NC_FILENAME = "250hpa_uv_2020.nc"

U_NAME = "u"
V_NAME = "v"

START_TIME = "2020-01-01T00:00:00"
END_TIME   = "2020-06-07T23:00:00"

STREAM_TO_VIDEO = True

FPS = 30
OUT_VIDEO = Path(f"./test_outputs/wind250hpa_particles_{START_TIME[:10]}_{END_TIME[:10]}_{FPS}fps.mp4")

coast_path = Path("./mask_outputs/coastlines.png")
coast_img = Image.open(coast_path).convert("L")  # grayscale
...
def main() -> None:
    ...
    # Slice time
    u = ds[U_NAME].sel(time=slice(START_TIME, END_TIME), plev=25000)
    v = ds[V_NAME].sel(time=slice(START_TIME, END_TIME), plev=25000)
    ...
    coast = np.array(coast_img)                      # (H,W) uint8
    if coast.shape[0] == 721:
        coast = np.delete(coast, DROP_LAT_ROW, axis=0)
    coast_mask = coast > 0
    assert coast_mask.shape == (H_img, W_img), (coast_mask.shape, (H_img, W_img))
    ...
    for i, t in enumerate(times):
        ...
        # ---- Overlay coastlines on top ----
        rgba[coast_mask, :3] = 0   # white coastlines
        rgba[coast_mask, 3]  = 0
